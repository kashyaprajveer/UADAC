...
class CriticNetwork(tf.keras.Model):
    ...
    def call(self, x):        # x is concat(action and state)
        s = self.fc1(x[1])
        s = self.concat([x[0],s])
        s = self.fc2(s)
        s = self.fc3(s)
        return self.out(s) # Returns q-value
...
class Critic2(object):
    ...
    def train(self, inputs, td_targets, weight_batch): # self.critic.train([batch_actions, batch_states], td_targets, weight_batch)
        weight_batch = tf.convert_to_tensor(weight_batch, dtype=tf.float32)
        with tf.GradientTape() as g:
            outputs = self.network(inputs) #call()
